...
ite_db_params = {
    "dbname": os.environ.get("ITE_DB_NAME"),
    "user": os.environ.get("ITE_DB_USERNAME"),
    "password": os.environ.get("ITE_DB_PASSWORD"),
    "host": os.environ.get("ITE_DB_HOSTNAME"),
...
def insert_dicom_metadata(
    ...
    try:
        ...
        cursor.execute(
            sql.SQL(
                "SELECT EXISTS(SELECT 1 FROM {table} WHERE mammography_id = %s)"
        ...
        if exists:
            print(
                f"mammography_id {mammography_id} already exists in the table {table_name}. No data inserted."
            )
        else:
            ...
            print(
                f"Data for mammography_id {mammography_id} successfully inserted into {table_name}."
            )
...
def write_oracle_s3(bucket_name, png_filepath):
    ...
    png_name = os.path.basename(png_filepath)
    with open(png_filepath, "rb") as file_stream:
        try:
            ...
            print(f"Saved {png_name} to oracle s3")
        except Exception as e:
            print(f"Error while saving to oracle s3: {e}")


def write_minio(bucket_name, png_filepath, png_image):
    # Save to minio
    client = Minio(
        f"{os.environ.get('MINIO_HOST')}:{os.environ.get('MINIO_PORT')}",
