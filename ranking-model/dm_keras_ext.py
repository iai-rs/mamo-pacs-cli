    ...
    def calc_test_auc(test_set, model, batch_size=None, test_samples=None,
        ...
        if isinstance(test_set, tuple):
            ...
            while samples_seen < test_samples:
                ...
                samples_seen += len(y)
                y_list.append(y)
                pred_list.append(augmented_predict(X,5))
                print(f"samples seen: {samples_seen}")
            y_true = np.concatenate(y_list)
            y_pred = np.concatenate(pred_list)
            # # update csv file - inference xlsx file (pos/neg)
            fnames = test_set.filenames
            fnames = [f.split('\\')[-1].rsplit('.', 1)[0] for f in fnames]
            dict_results = dict(zip(fnames,y_pred))
            if len(wei_list) > 0:
